...
class FrameSegment(object):
    ...
    def __init__(self, sock, port, addr="10.18.7.157"):
        self.s = sock
        self.port = port
        self.addr = addr
...
def main():
    ...
    while (cap.isOpened()):
        ...
        #frame = cv2.inRange(frame, (H_low, S_low, V_low), (H_high, S_high, V_high))
       ...
       #
        try:
            ...
            x, y, w, h = cv2.boundingRect(cnt[order])

            if w > 2.2 * h:
                print("BUMPER")

                order = order + 1
            ((xx, yy), rr) = cv2.minEnclosingCircle(cnt[order])
